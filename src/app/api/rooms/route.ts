  ...
  try {
    ...
    const roomsWithBeds = rooms.map((room) => {
      const beds = Array.from({ length: room.capacity }, (_, i) => {
        ...
        return { bedNo, isOccupied: !!student, studentId: student?.id, studentName: student?.fullName };
      });
      return { ...room, occupied: room.students.length, beds };
    });

    const filtered =
      filter === "available" ? roomsWithBeds.filter((r) => r.occupied < r.capacity) :
      filter === "full"      ? roomsWithBeds.filter((r) => r.occupied === r.capacity) :
                               roomsWithBeds;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const capacityMap: Record<string, number> = { "2-bed": 2, "4-bed": 4, "6-bed": 6 };
    const typeMap: Record<string, string>      = { "2-bed": "TWO_BED", "4-bed": "FOUR_BED", "6-bed": "SIX_BED" };

    const room = await prisma.room.create({
      data: {
        roomNumber: body.roomNumber,
        floor:      body.floor,
        type:       typeMap[body.type] as any,
