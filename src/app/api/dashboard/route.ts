...
export async function GET(_req: NextRequest) {
  try {
    ...
    const [
      totalStudents, totalBeds, pendingFees, openComplaints,
      visitorsInside, activeNotices,
      recentStudents, recentComplaints, recentVisitors, recentNotices,
    ] = await Promise.all([
    ...
    const activityFeed = [
      ...recentStudents.map((s) => ({
        id: `student-${s.id}`, action: "New student checked in",
        description: `${s.fullName} — Room ${s.room.roomNumber}`,
        timestamp: s.createdAt.toISOString(), type: "student",
      })),
      ...recentComplaints.map((c) => ({
        id: `complaint-${c.id}`, action: "Complaint filed",
        description: `${c.student.fullName} — ${c.category} (Room ${c.room.roomNumber})`,
        timestamp: c.createdAt.toISOString(), type: "complaint",
      })),
      ...recentVisitors.map((v) => ({
        id: `visitor-${v.id}`, action: "Visitor logged in",
        description: `${v.visitorName} visiting ${v.student?.fullName ?? "hostel"}`,
        timestamp: v.entryTime.toISOString(), type: "visitor",
      })),
      ...recentNotices.map((n) => ({
        id: `notice-${n.id}`, action: "Notice published",
        description: `${n.title} [${n.priority}]`,
        timestamp: n.publishedAt.toISOString(), type: "notice",
      })),
    ]
